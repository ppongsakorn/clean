    ...
    <div class="flex-1 overflow-auto">
      ...
      <div v-else class="p-4 space-y-4">
        ...
        >
          <!-- Group header -->
          <div
            class="flex items-center gap-2 px-2 py-1.5 rounded-md group cursor-pointer hover:bg-elevated transition-colors"
          ...
          <template v-if="!collapsed[status.id]">
            <div
              ...
              :key="task.id"
              class="flex items-center gap-2 px-2 py-2 ml-4 rounded-md hover:bg-elevated transition-colors group cursor-pointer"
            >
              <!-- Status dot -->
              <button
                class="w-3.5 h-3.5 rounded-sm border-2 shrink-0 transition-colors hover:opacity-80"
...
import { Filter, Users, CheckSquare, Plus, ChevronRight } from 'lucide-vue-next'
...
const { data: tasksData, refresh: refreshTasks } = await useFetch<any[]>(`/api/projects/${projectId}/tasks`)
...
async function saveTask(status: any) {
  ...
  try {
    ...
    newTaskTitle.value = ''
    // keep the row open for rapid entry
    nextTick(() => inputRefs[status.id]?.focus())
  } catch {
    // stay open so user doesn't lose input
  }
}
</script>
